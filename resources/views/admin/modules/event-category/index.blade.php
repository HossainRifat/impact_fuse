...
<div class="card body-card">
    <div class="card-body">
        @include('admin.modules.event-category.partials.search')
        <table class="table table-striped table-hover">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{$category->slug}}</td>
                    <td class="text-center">
                        @if($category->status == \App\Models\EventCategory::STATUS_ACTIVE)
                        <x-active :status="$category->status" />
                        @else
                        <x-inactive :status="$category->status" :title="'Inactive'" />
                        {{\App\Models\EventCategory::STATUS_LIST[$category->status] ?? null}}
                        @endif
                    </td>
                    <td>
                    ...
                    <td>
                        <div class="d-flex justify-content-center">
                            <x-view-button :route="route('event-category.show', $category->id)"/>
                            <x-edit-button :route="route('event-category.edit', $category->id)"/>
                            <x-delete-button :route="route('event-category.destroy', $category->id)"/>
                        </div>
                    </td>
                </tr>
                @empty
